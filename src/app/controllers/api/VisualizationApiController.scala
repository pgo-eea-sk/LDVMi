...
class VisualizationApiController(implicit inj: Injector) extends ApiController with Injectable {
  ...
  def dataReferences(id: Long) = Action.async { implicit rs =>
    withEvaluation(id) { evaluation =>
      Ok(Json.toJson(visualizationService.dataReferences(evaluation)))
    }
  }

  def queries(id: Long, permalinkToken: String) = Action.async { implicit rs =>
    withEvaluation(id) { evaluation =>
  ...
  def customCube(id: Long, permalinkToken: String, dimensionUri: String, valueUri: String) = Action.async { implicit rs =>
